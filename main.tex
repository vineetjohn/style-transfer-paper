...
\begin{abstract}
	...
	Our code and data is publicly available for replicability and extension purposes.
	\footnote{\url{https://bit.ly/2BGnypI}}
...
\begin{equation} \label{eqn:class-pred}
	y = \sigma(\bm w_\text{smul}^\top \bm s + b_\text{smul})
\end{equation}
where $\bm\theta_\text{smul}=[\bm w_\text{smul}; b_\text{smul}]$ are the classifier's parameters for multi-task learning, and $y$ is the predicted label distribution.

This is trained using a simple cross-entropy loss
\begin{equation} \label{eqn:style-multi-task-loss}
	\loss{smul}(\bm\theta_{E};\bm\theta_\text{smul}) =
	- \mathbb{E} [\log p(y' | \bm s; \bm\theta_\text{smul})]
\end{equation}
...
\begin{equation}
	y = \sigma(\bm w_\text{sdis}^\top \bm c + b_\text{sdis})
\end{equation}
where $\bm\theta_\text{sdis}=[\bm w_\text{sdis}; b_\text{sdis}]$ are the parameters of the adversary, and $y$ is the predicted label distribution.

It is trained by minimizing the following objective, using a cross-entropy loss.
\begin{equation} \label{eqn:adv-disc-loss}
	\loss{sdis}(\bm\theta_\text{sdis}) =
	- \mathbb{E} [\log p(y' | \bm c; \bm\theta_\text{sdis})]
\end{equation}
...
\begin{equation}
	\loss{sadv}(\bm\theta_E)=\mathcal{H}(y|\bm c; \bm\theta_\text{sdis})
\end{equation}
...
The input sentences $\rmx$ are represented as vectors of the same size as the corpus vocabulary, with each index of the vector denoting the probability of a word's presence in the sentence.
...
\begin{equation} \label{eqn:bow-pred}
	v = \sigma(\bm w_\text{cmul}^\top \bm s + b_\text{cmul})
\end{equation}
where $\bm\theta_\text{cmul}=[\bm w_\text{cmul}; b_\text{cmul}]$ are the classifier's parameters for multi-task learning, and $v$ is the predicted BoW distribution.

This is trained using a simple cross-entropy loss
\begin{equation} \label{eqn:content-multi-task-loss}
	\loss{cmul}(\bm\theta_{E};\bm\theta_\text{cmul}) =
	- \mathbb{E} [\log p(v' | \bm s; \bm\theta_\text{cmul})]
\end{equation}
where $\bm\theta_E$ are the encoder's parameters, and $v'$ is the true BoW distribution.
...
The discriminator uses a softmax activation to predict the occurrence probability of each word in the original sentence, between $0$ and $1$.
\begin{equation}
	v = \sigma(\bm w_\text{cdis}^\top \bm s + b_\text{cdis})
\end{equation}
where $\bm\theta_\text{cdis}=[\bm w_\text{cdis}; b_\text{cdis}]$ are the classifier's parameters for BoW prediction, and $v$ is the predicted BoW distribution.
...
\begin{equation} \label{eqn:adv-bow-disc-loss}
	\loss{cdis}(\bm\theta_\text{cdis}) =
	- \mathbb{E} [\log p(v' | \bm s; \bm\theta_\text{cdis})]
\end{equation}
where $v'$ is the true BoW distribution.

We also refrain from propagating the effects of this discriminator loss $\loss{cdis}$ to the encoder parameters, ensuring that the parameters that each adversary and the autoencoder can update are mutually exclusive.
...
\begin{equation}
	\loss{cadv}(\bm\theta_E) = \mathcal{H}(v | \bm s; \bm\theta_\text{cdis})
\end{equation}
...
\begin{align*}
	\loss{ovr} =
	 & \loss{rec}(\bm\theta_E, \bm\theta_D)                                                                                 \\
	 & + \lambda_\text{smul} \loss{smul} (\bm\theta_E,\bm\theta_\text{smul}) - \lambda_\text{sadv} \loss{sadv}(\bm\theta_E) \\
	 & + \lambda_\text{cmul} \loss{cmul} (\bm\theta_E,\bm\theta_\text{cmul}) - \lambda_\text{cadv} \loss{cadv}(\bm\theta_E)
\end{align*}
where $\lambda_\text{smul}$, $\lambda_\text{sadv}$ and $\lambda_\text{cadv}$ balance the model's auxiliary losses.
...
\begin{algorithm}[ht]
	\While{epochs remaining}{
		minimize $\loss{sdis}(\bm\theta_\text{sdis})$ w.r.t. $\bm\theta_\text{sdis}$\;
		minimize $\loss{cdis}(\bm\theta_\text{cdis})$ w.r.t. $\bm\theta_\text{cdis}$\;
		minimize $\loss{ovr}$ w.r.t. $\bm\theta_E, \bm\theta_D, \bm\theta_\text{smul}, \bm\theta_\text{cmul}$\;
	}
...
Both the autoencoder and the discriminators are trained once per epoch with $\lambda_\text{smul} = 1$, $\lambda_\text{cmul} = 1$, $\lambda_\text{sadv} = 0.3$ and $\lambda_\text{cadv} = 0.001$.
...
\begin{table*}[ht]
	...
	\begin{tabular}{| l || c | c | c | c |}
		...
		\hline
		$\loss{rec}$, $\loss{smul}$                                              & 0.789           & 0.898               & 0.259          & -14.56          \\
		\hline
		$\loss{rec}$, $\loss{cmul}$                                              & 0.116           & 0.947               & 0.595          & -12.00          \\
		\hline
		$\loss{rec}$, $\loss{sadv}$                                              & 0.727           & 0.880               & 0.204          & -14.16          \\
		\hline
		$\loss{rec}$, $\loss{cadv}$                                              & 0.168           & 0.915               & 0.328          & -14.45          \\
		\hline
		$\loss{rec}$, $\loss{smul}$, $\loss{sadv}$                               & 0.890           & 0.882               & 0.211          & -14.41          \\
		\hline
		$\loss{rec}$, $\loss{smul}$, $\loss{cmul}$                               & 0.660           & 0.941               & 0.554          & -12.47          \\
		\hline
		$\loss{rec}$, $\loss{smul}$, $\loss{cadv}$                               & 0.783           & 0.896               & 0.257          & -14.34          \\
		\hline
		$\loss{rec}$, $\loss{sadv}$, $\loss{cmul}$                               & 0.733           & 0.910               & 0.446          & -11.53          \\
		\hline
		$\loss{rec}$, $\loss{sadv}$, $\loss{cadv}$                               & 0.749           & 0.883               & 0.202          & -14.36          \\
		\hline
		$\loss{rec}$, $\loss{cmul}$, $\loss{cadv}$                               & 0.114           & 0.949               & 0.595          & -12.29          \\
		\hline
		$\loss{rec}$, $\loss{smul}$, $\loss{sadv}$, $\loss{cmul}$                & 0.877           & 0.899               & 0.402          & -10.19          \\
		\hline
		$\loss{rec}$, $\loss{smul}$, $\loss{sadv}$, $\loss{cadv}$                & 0.885           & 0.878               & 0.197          & -14.05          \\
		\hline
		$\loss{rec}$, $\loss{smul}$, $\loss{cmul}$, $\loss{cadv}$                & 0.657           & 0.939               & 0.545          & -12.39          \\
		\hline
		$\loss{rec}$, $\loss{sadv}$, $\loss{cmul}$, $\loss{cadv}$                & 0.759           & 0.909               & 0.439          & -10.92          \\
		\hline
		$\loss{rec}$, $\loss{smul}$, $\loss{sadv}$, $\loss{cmul}$, $\loss{cadv}$ & 0.887           & 0.888               & 0.290          & -12.95          \\
		\hline
	\end{tabular}
...
Table~\ref{tab:ablation-results} presents the results of an ablation test.
...
\begin{figure}[h]
	...
	\caption{t-SNE Plots: VAE with $\loss{rec} + \loss{sadv}$}
...
\begin{figure}[h]
	...
	\caption{t-SNE Plots: VAE with $\loss{rec} + \loss{smul}$}
