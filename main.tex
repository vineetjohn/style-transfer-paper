...
\begin{table*}[!t]
	...
	\begin{tabular}{| l || c | c | c | c |}
		...
		\hline
		Ours (VAE)                                 & \textbf{0.934}           & \textbf{0.904}             & \textbf{0.473}          & \textbf{-9.84}           \\
		\hline
	\end{tabular}
	\caption{Comparison using the Yelp dataset. The style-embedding approach is achieves poor transfer accuracy, and should not be considered as an effective4 style-transfer model. Despite this, our model outperforms other previous methods in terms of all aspects (transfer length, content preserving, and language fluency).}
...
\begin{table*}[!t]
	...
	\begin{tabular}{| l || c | c | c | c |}
		...
		\tabc{2}{Model}                            & \tabh{Transfer} & \tabh{Cosine}     & \tabh{Word}    & \tabh{Language} \\
		                                           & \tabh{Strength} & \tabh{Similarity} & \tabh{Overlap} & \tabh{Fluency}  \\
		\hline
		\hline	Style Embedding \cite{fu2018style}         & 0.417           & 0.933             & 0.359          & -28.13          \\
		\hline\hline
