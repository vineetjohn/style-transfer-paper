...
Having trained an adversary, we would like the autoencoder to be tuned in such an \textit{ad hoc} fashion that $\bm c$ is not discriminative in style. {\color{red} In exiting literature, there could be different approaches, for example, maximizing the adversary's loss~\cite{} or penalizing the entropy  of the adversary's prediction~\cite{}.} In our work, we adopt the latter, as it can be easily extended to multi-category classification, used for content-oriented loss in some other part of our approach. Formally, the adversarial loss for the style, denoted as ``adv(s)''
\begin{equation}
	\loss{adv(s)}(\nnweight{E})=\mathcal{H}(\bm y|\bm c; \nnweight{dis(s)})\label{eqn:advs}
\end{equation}
where $\mathcal{H}(\bm p)=-\sum_{i\in\text{labels}}p_i\log p_i$ and $\bm y=[y, 1-y]$ is the predicted distribution over the style labels (positive and negative).

In this phase, Equation~\ref{eqn:advs} is maximized with respect to the encoder. It is viewed as a function of $\nnweight{E}$, and $\nnweight{dist(s)}$ is fixed. Since the 
