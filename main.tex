...
\begin{abstract}
	...
	\footnote{\url{https://bit.ly/2wxps69} Our code and data are publicly available at this URL. The materials are properly anonymized during double-blind review.}
...
Such transformations, however, make intermediate features ``latent'', in the sense that, they do not have explicit meaning and are not explainable.
...
Both qualitative and quantitative results show that both the style and content vectors are indeed disentangled well.
In the style-transfer evaluation, we achieve significantly better style-transfer strength and language fluency compared with previous results, while obtaining comparable content preservation scores.
...
\end{equation}
where $\hyp{kl}$ is the hyperparameter balancing the reconstruction loss and the KL terms. $p(\bm h)$ is the prior, set to the standard normal distribution $\mathcal{N}(\bm 0,\mathrm I)$. The motivation for using VAE as opposed to DAE is  that the reconstruction is based on the samples of the posterior, which theoretically populates the neighborhood and thus smooths the latent space. \citeay{bowman2016generating} show that VAE generates more fluent sentences than DAE.
...
The above multi-task loss could only ensure the style space does contain style information.
However, the content space might also contain style information, which is undesired for disentanglement and style transfer.
...
In our work, we adopt the latter, as it can be easily extended to multi-category classification, used for content-oriented loss in some other part of our approach. Formally, the adversarial objective for the style is to maximize
\begin{equation}
...
\begin{equation}\label{eqn:content-multi-task-loss}
	\loss{mul(c)}(\nnweight{E};\nnweight{mul(c)}) = - \sum_w t_c(w)\log y_c(w)
\end{equation}
...
\begin{align}
	...
	\loss{dis(c)}(\nnweight{dis(c)}) & = - \sum_w t_c(w)\log y_c(w)
\end{align}
...
We also use an Amazon review dataset~\footnote{\url{http://jmcauley.ucsd.edu/data/amazon/}}, following \citeay{fu2018style}.
...
The word embedding layer is initialized using pre-trained Word2Vec embeddings \cite{mikolov2013distributed} from the respective training sets.
...
These hyperparameters were identified by performing a log-scale grid search within two orders of magnitude of the default value of $1$, and choosing the hyperparameters that yielded the best results.
...
A convolutional neural network (CNN) style classifier \cite{kim2014convolutional} is trained to predict the style of the generated sentences.
...
We compute a sentence embedding by concatenating the min, max, and average of its constituent word embeddings \cite{fu2018style}.
...
A simpler unigram overlap metric is also utilized to evaluate the similarity between the content of the source and generated sentences.
Given a source sentence $x$ and an style-transferred sentence $y$, let $w_x$ and $w_y$ be the set of unique words present in $x$ and $y$ respectively, while excluding sentiment words \cite{hu2004mining} and stopwords \cite{bird2004nltk}.
...
We use a trigram Kneser-Ney smoothed language model \cite{kneser1995improved} as a quantifiable and automated scoring metric by which we assess the quality of generated sentences.
...
This language model is trained on the complete corpus that we evaluate our style-transfer models with.
...
Manual evaluations are performed by randomly sampling 40 pairs of original and style-transferred sentences from each model trained on the Yelp dataset, and requesting 6 human annotators to rate them on a 1-5 Likert scale \cite{stent2005evaluating} based on their transfer strength, content preservation and language quality.
...
These results verify the effectiveness of our disentangling approach, because the style space contains style information, whereas the content space does not.
...
As can be seen from the t-SNE plots, sentences with different styles are noticeably separated in a clean manner in the style space (LHS), but are indistinguishable in the content space (RHS).
...
The disentangled latent space is applied to a style-transfer sentence generation task, where the goal is to generate a sentence with different sentiment.
...
\begin{table}[ht]
	...
	\begin{tabular}{| l || c | c | c | }
		...
		\hline
		\hline
		\citeauthor{shen2017style}         & 3.63            & 3.07                & 3.08            \\
		\hline
		\citeauthor{fu2018style}           & 1.67            & \textbf{3.84}       & 3.66            \\
		\hline
		\citeauthor{zhao2018adversarially} & 3.55            & 3.09                & 3.77            \\
		\hline
