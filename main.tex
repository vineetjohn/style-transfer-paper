...
\begin{abstract}
	...
	We achieve similar content preservation scores compared to previous state-of-the-art approaches, and significantly better style-transfer strength scores.\footnote{Our code and data are publicly available at \url{https://bit.ly/2wxps69} (The materials are properly anonymized during double-blind review.)}
...
The adversarial loss, on the contrary, minimizes the predictability of information that should not be contained in that space. In previous work, researchers typically work with the style, or specifically, sentiment space~\cite{},
 but simply ignore the content space, as it is hard to formalize what ``content'' actually refers to. 
...
Recently, \cite{} treat the formality of writing as a style, and create a parallel corpus for style transfer with sequence-to-sequence models. This is beyond the scope of our paper, as we focus on non-parallel text style transfer.
% http://aclweb.org/anthology/N18-1012
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.7\linewidth]{model-overview-training}
	\label{fig:model-training-overview}

	 \includegraphics[width=0.7\linewidth]{model-overview-inference}
...
Suppose at a time step $t$, the decoder RNN predicts the word $x_t$ with probability $p(x_t|\bm h, x_1\cdots x_{t-1})$. Then the autoencoder is trained using a sequence-aggregated cross-entropy loss, given by
\begin{equation}
	\loss{AE}(\nnweight{E},\nnweight{D})= -\sum_{t=1}^n \log p(x_t|\bm h, x_1\cdots x_{t-1})
\end{equation}
where $\nnweight{E}$ and $\nnweight{D}$ are the parameters of the encoder and decoder, respectively. Both the encoder and decoder are deterministic functions in the original autoencoder model~\cite{}, and thus we call it an \textit{deterministic autoencoder} (DAE).

\subsubsection{Variational Autoencoder}

In addition to the deterministic auto-encoding objective presented above, we also tried a variational autoencoder (VAE) \cite{kingma2013auto}, which imposes a probabilistic distribution to the latent vector. Then, the Kullback-Leibler (KL) divergence \cite{kullback1951information} is penalized to the regularize the latent space.
...
\begin{align} \label{eqn:vae-rec}
	\loss{AE}(\nnweight{E}, \nnweight{D}) = \nonumber
	 ...
	 & + \hyp{kl(s)}\operatorname{KL}(q_{E}(\bm s|\rmx)\|p(\bm s)) \nonumber                                      \\
	 & + \hyp{kl(c)}\operatorname{KL}(q_{E}(\bm c|\rmx)\|p(\bm c))
\end{align}
where $\hyp{kl(s)}$ and $\hyp{kl(c)}$ are the weights for each of the KL divergence terms, and $p(\bm c)$ and $p(\bm s)$ are the priors, usually set to the standard normal distribution $\mathcal{N}(\bm 0,\mathrm I)$.
...
\begin{table*}[ht]
	...
	\begin{tabular}{| l || c | c | c | c |}
		...
		\hline
		$\loss{rec}$, $\loss{adv(c)}$                                                    & 0.103           & 0.940             & 0.470          & -12.92          \\
		\hline
		$\loss{rec}$, $\loss{mul(s)}$, $\loss{mul(c)}$                                   & 0.822           & 0.928             & 0.561          & -11.32          \\
		\hline
		$\loss{rec}$, $\loss{mul(s)}$, $\loss{adv(s)}$                                   & 0.912           & 0.866             & 0.171          & -9.59           \\
		\hline
		$\loss{rec}$, $\loss{mul(s)}$, $\loss{adv(c)}$                                   & 0.781           & 0.906             & 0.318          & -11.99          \\
		\hline
		$\loss{rec}$, $\loss{mul(c)}$, $\loss{adv(s)}$                                   & 0.825           & 0.894             & 0.507          & -10.94          \\
		\hline
		$\loss{rec}$, $\loss{mul(c)}$, $\loss{adv(c)}$                                   & 0.149           & 0.929             & 0.621          & -11.22          \\
		\hline
		$\loss{rec}$, $\loss{adv(s)}$, $\loss{adv(c)}$                                   & 0.780           & 0.884             & 0.214          & -12.03          \\
		\hline
		$\loss{rec}$, $\loss{mul(s)}$, $\loss{mul(c)}$, $\loss{adv(s)}$                  & 0.922           & 0.903             & 0.476          & -9.65           \\
		\hline
		$\loss{rec}$, $\loss{mul(s)}$, $\loss{mul(c)}$, $\loss{adv(c)}$                  & 0.844           & 0.925             & 0.556          & -11.13          \\
		\hline
		$\loss{rec}$, $\loss{mul(s)}$, $\loss{adv(s)}$, $\loss{adv(c)}$                  & 0.934           & 0.864             & 0.157          & -10.11          \\
		\hline
		$\loss{rec}$, $\loss{mul(c)}$, $\loss{adv(s)}$, $\loss{adv(c)}$                  & 0.806           & 0.896             & 0.500          & -9.98           \\
		\hline
		$\loss{rec}$, $\loss{mul(s)}$, $\loss{mul(c)}$, $\loss{adv(s)}$, $\loss{adv(c)}$ & 0.934           & 0.904             & 0.473          & -9.84           \\
		\hline
	\end{tabular}
