...
We build a classifier on the style space that predicts the style label. Formally, a softmax layer is applied to the style vector $\bm s$, given by
...
The Yelp review dataset \cite{challenge2013yelp} has been sourced from the code repository accompanying the implementation of \citeay{shen2017style}~\footnote{\url{https://github.com/shentianxiao/language-style-transfer}}.
...
An Amazon review dataset~\footnote{\url{http://jmcauley.ucsd.edu/data/amazon/}}, is also used, following \citeay{fu2018style}.
...
These hyperparameters were identified by performing a log-scale grid search within two orders of magnitude of the default value of $1$, and choosing those that yielded the best results.
...
We evaluate our method using four metrics: style-transfer strength, cosine similarity, word overlap, and language fluency. We use these metrics to compare our model to previous state-of-the-art work \cite{shen2017style,fu2018style,zhao2018adversarially} empirically.
...
We compute a sentence embedding by concatenating the $\operatorname{min}$, $\operatorname{max}$, and $\operatorname{mean}$ of its constituent word embeddings \cite{fu2018style}.
Then, the cosine similarity between the source and generated sentence embeddings is computed to evaluate how close they are in meaning, which is an indicator of content preservation.
...
Manual evaluations are performed by randomly sampling 40 pairs of original and style-transferred sentences from each evaluated model trained on the Yelp dataset, and requesting 6 human annotators to rate them on a 1-5 Likert scale \cite{stent2005evaluating} based on their transfer strength, content preservation and language quality.
...
We compare our approach with previous state-of-the-art work in Tables \ref{tab:yelp-comparison-previous} and \ref{tab:amazon-comparison-previous}.
We replicated the experiments with their publicly available code.
