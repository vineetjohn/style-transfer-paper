...
{\color{red}The Yelp review dataset \cite{challenge2013yelp} has been sourced from the code repository accompanying the implementation of \citeay{shen2017style}.\footnote{\url{https://github.com/shentianxiao/language-style-transfer}}
}
...
We also used the Amazon review dataset,\footnote{\url{http://jmcauley.ucsd.edu/data/amazon/}} following previous work~\citeay{fu2018style}.
...
We used the Adam optimizer \cite{kingma2014adam} for the autoencoder and the RMSProp optimizer \cite{tieleman2012lecture} for the discriminators, each with an initial learning rate of $10^{-3}$. We trained our model for 20 epochs.
...
We set $\hyp{kl(s)} = 0.03$ and $\hyp{kl(c)} = 0.03$ and use the $\operatorname{sigmoid}$ KL-weight annealing schedule following \citeay{bahuleyan2018probabilistic}. {\color{blue} They were tuned with the original VAE model, and we directly use them for training disentangled models.}

\subsection{Evaluation Metrics}
We shall evaluate all 
\cite{shen2017style,fu2018style,zhao2018adversarially} empirically.

\subsection{Style-Transfer Strength}
...
\subsection{ExperimentI: Disentangling Latent Space}
