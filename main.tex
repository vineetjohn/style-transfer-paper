...
Similar to the multi-task style prediction objective, thee also try to predict the content, represented by a bag-of-words (BoW), from the content vector $\bm c$.
...
The overall loss $\loss{ovr}$, used for the autoencoder,  comprises several distinct objectives: the reconstruction objective, the multi-task objectives for style and content, and the adversarial objectives for style and content.
\begin{align}
	\loss{ovr} = & \loss{AE}(\nnweight{E}, \nnweight{D})  \nonumber                                                                   \\
...
To put it all together, the model training involves an alternation of optimizing adversary's losses $\loss{dis(s)}$ and $\loss{dis(c)}$, and the model's own loss, shown in Algorithm~\ref{alg:training-process}.
...
\end{equation*}
The inferred target style $\hat{\bm s}$ is concatenated with the encoded content $\bm c^*$ for decoding style-transferred sentences (Figure~\ref{fig:overview}b).
